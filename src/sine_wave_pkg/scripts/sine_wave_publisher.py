...
import math
from sine_wave_parameters import sine_wave_params
...
class SineWavePublisher(Node):
    ...
    def sine_wave_publish_message(self):
        ...
        msg = Float64()
        msg.data = self.amplitude * math.sin(
            self.angular_frequency * self.angle + self.phase
        )
    ...
    def parameter_callback(self, params):
        for param in params:
            ...
            self.get_logger().info(
                "Parameter %s has been set to: %s" % (param.name, param.value)
            )
